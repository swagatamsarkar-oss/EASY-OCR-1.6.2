...
class LmdbDataset(Dataset):
    ...
    def __init__(self, root, opt):
        ...
        if not self.env:
            raise ValueError(f'Cannot open LMDB from {root}')
        
        with self.env.begin(write=False) as txn:
            num_samples = txn.get('num-samples'.encode())
            if num_samples:
                self.nSamples = int(num_samples.decode('utf-8'))
            else:
                raise ValueError(f'num-samples not found in {root}')
    ...
    def __getitem__(self, index):
        ...
        with self.env.begin(write=False) as txn:
            label_key = f'label-{actual_index:09d}'.encode()
            label = txn.get(label_key).decode('utf-8')
            
            img_key = f'image-{actual_index:09d}'.encode()
            ...
            try:
                ...
                print(f'Corrupted image for index {actual_index}')
...
def hierarchical_dataset(root, opt, select_data='/'):
    """PATCHED: Support both LMDB and CSV formats"""
    dataset_list = []
    dataset_log = f'dataset_root: {root}\t'
    
    selected_d_list = select_data if isinstance(select_data, list) else select_data.split('-')
    
    for selected_d in selected_d_list:
        dataset_path = root if selected_d == '/' else os.path.join(root, selected_d)
        dataset_log += f'dataset: {selected_d}\n{dataset_path}\n'
        
        # Check for LMDB format (data.mdb file)
        if os.path.exists(os.path.join(dataset_path, 'data.mdb')):
            print(f'✓ Loading LMDB: {dataset_path}')
            dataset = LmdbDataset(dataset_path, opt)
            dataset_list.append(dataset)
        else:
            # Scan for LMDB in subdirectories or CSV format
            for dirpath, dirnames, filenames in os.walk(dataset_path):
                if 'data.mdb' in filenames:
                    print(f'✓ Found LMDB in: {dirpath}')
                    dataset = LmdbDataset(dirpath, opt)
                    dataset_list.append(dataset)
                elif 'labels.csv' in filenames:
                    print(f'✓ Found CSV in: {dirpath}')
